...
{
	selmon->gappx = MAX(selmon->gappx + arg->i, 0);
	tilegap(selmon);
}
...
nametag(const Arg *arg) {
    ...
    // Edit - Allow modification to dmenu command.
    char cmd[1000];
    for(i=0; i < (int)LENGTH(nametagcmd); i++)
    {
        if(i == 0)
            strcpy(cmd, nametagcmd[i]);
        else
        {
            strcat(cmd, " ");
            strcat(cmd, nametagcmd[i]);
        }
    }
    strcat(cmd, " < /dev/null");
